...
export default {
    addToSprint: (state, index) => {
        let sprint = (state.session.project.sprints) ? state.session.project.sprints[state.session.project.current.sprintIndex] : false
        ...
        state.session.project.sprints = (state.session.project.sprints) ? state.session.project.sprints : []
        if (!sprint.list) {
            sprint.list = []
        }
